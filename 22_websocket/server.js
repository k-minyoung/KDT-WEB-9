...
wss.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        //웹소켓을 통해 클라이언트와 서버간의 데이터를 주고받을때는
        //일반적으로 문자열 또는 버퍼형태로 전달됨.
        //서버가 모두 다른환경이기 때문에 객체를 전달할때는 객체를
        //일련의 바이트로 변환하는 직렬화 과정이 필요 = 버퍼를 쓰는 이유
      //  console.log(message); //버퍼값
        const msg = JSON.parse(message)
        console.log(`클라이언트로부터 받은 메세지 : ${msg.message}`) //백틱은 자동으로 변환시켜줌
        //socket.send(`서버메세지 : ${message}`)
        sockets.forEach( (elem) => {//elem = socket
