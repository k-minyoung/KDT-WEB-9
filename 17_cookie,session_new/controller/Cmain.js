...
//회원 전체조회
const list = (req,res) => {
    if( req.session.userInfo) { //로그인 했는지 체크. 로그인하지 않았으면 userInfo가 없기때문에
...
const post_signin = async (req, res) => {
    ...
    if (user) {
        ...
        if (result) {
            //세션생성
            req.session.userInfo = {name : user.name, id: user.id}//로그인 하자마자 세션 생성
            
            res.cookie('login',true)
            res.json({ result: true, data: user });
        } else {
            // res.cookie('login','loginfalse')
